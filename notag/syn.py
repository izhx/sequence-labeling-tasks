    ...
    def forward(self, input, masks, initial=None):
        ...
        for layer in range(self.num_layers):
            ...
            layer_output, (layer_h_n, layer_c_n) = MyLSTM._forward_rnn(
                cell=self.fcells[layer], input=input, masks=masks, initial=initial, drop_masks=hidden_mask)
            if self.bidirectional:
                blayer_output, (blayer_h_n, blayer_c_n) = MyLSTM._forward_brnn(
                    cell=self.bcells[layer], input=input, masks=masks, initial=initial, drop_masks=hidden_mask)
...
class ParserModel(nn.Module):
    def __init__(self, vocab, config, pretrained_embedding):
        ...
        self.dims = [2*config.lstm_hiddens] * config.lstm_layers + [self.mlp_arc_dep.hidden_size] * 2

    def forward(self, words, extwords, masks, projections=None):
        # x = (batch size, sequence length, dimension of embedding)
        x_word_embed = self.word_embed(words)
        x_extword_embed = self.extword_embed(extwords).to(words.device)
        x_embed = x_word_embed + x_extword_embed

        if self.training:
            x_embed = drop_uni_input_independent(x_embed, self.config.dropout_emb)

        outputs, (*_, hiddens) = self.lstm(x_embed, masks.float(), None)
        ...
        outputs = [h.transpose(1, 0) for h in hiddens] + [x_all_dep, x_all_head]
        if projections:
            outputs = [m(o) for m, o in zip(projections, outputs)]
