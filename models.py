...
from metric import SRLMetric, DependencyParsingMetric
...
class SemanticRoleLabeler(Model):
    ...
    def forward(self,
        ...
        if self.depsawr_forward:
            dep_embs = self.depsawr_forward(kwargs['dw'], kwargs['ew'], mask, self.projections)
        feat = torch.cat([feat, *emb, *dep_embs], dim=-1)
